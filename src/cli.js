...
async function retrieveEnvData(env, app, returnJson = true){
  const {stdout: data} = await exec(`aws elasticbeanstalk describe-configuration-settings --environment-name ${env} --application-name ${app}`);
  if(returnJson){
...
export function cli (){
  ...
  retrieveEnvData(cliOptions.environmentName, cliOptions.applicationName, true).then((data) => {
